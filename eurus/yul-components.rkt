...
;   - in the current setting, values can only be a non-empty list or (void)
;     - (void) means whatever executed has side effects only and does not return anything
...
(struct callpack (caller gas wei calldata) #:transparent #:reflection-name 'callpack)
...
(struct param (
    ...
    ; parameters for a callpack instance
    ; - mode: inherit
    ; - default: null
    callpack
...
(define (copy-param obj-param #:code-scope [code-scope null] #:callpack [callpack null] #:new-block-scope [new-block-scope null])
    (define code-scope-0 (if (null? code-scope) (param-code-scope obj-param) code-scope)) ; inherit
    (define callpack-0 (if (null? callpack) (param-callpack obj-param) callpack)) ; inherit
    (define new-block-scope-0 (if (null? new-block-scope) #t new-block-scope)) ; reset
    (param code-scope-0 callpack-0 new-block-scope-0)
)
(define (default-param) (param 'start null #t))