...
(define manager%
    (class object%
        ...
        (define/public (call arg-address arg-callpack [arg-codescope 'dispatcher])
            ...
            (define v0 (comp:callpack-wei arg-callpack)) ; value to send
            (define caller0 (comp:callpack-caller arg-callpack)) ; this is the caller, but arg-address is the callee
            ...
            (cond
                [(empty? (comp:callpack-calldata arg-callpack))
                ...
                [else
                    ...
                    (cond
                        [(equal? 'constructor arg-codescope)
                            ; call constructor
                            (send vm0 construct arg-callpack)
                        ]
                        [(equal? 'dispatcher arg-codescope)
                            ; directly return the memory
                            (send vm0 dispatch arg-callpack)
                        ]
